...
public class CanEat {
    ...
    public boolean[] canEat2(int[] candiesCount, int[][] queries) {
        long[] preSum = new long[candiesCount.length];//吃完多少个糖果才到第i种糖果
        long sum = 0;
        for (int i = 0; i < candiesCount.length; i++) {
            preSum[i] = sum;
            sum += candiesCount[i];
        ...
        for (int i = 0; i < queries.length; i++) {
            ...
            long left = day, right = (long)day * cap;
            
            result[i] = left <= preSum[type] + candiesCount[type]
                        && preSum[type] + 1 <= right;
        }
        return result;
    }
